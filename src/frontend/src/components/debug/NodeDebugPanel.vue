...
<template>
  <section class="node-debug-panel">
    ...
    <div class="node-debug-panel__actions">
      <button v-if="nodeData.nodeType === 'sql_query'" @click="handleValidate">Validate</button>
      <button v-if="nodeData.nodeType === 'sql_query'" @click="handlePreview">Preview</button>
    ...
    <TablePreview v-if="previewRendererResult && nodeData.nodeType !== 'chart_output'" :result="previewRendererResult" mode="preview" />
    <ChartPreview v-if="nodeData.nodeType === 'chart_output'" :result="previewRendererResult ?? { kind: 'EMPTY' }" mode="preview" />
  </section>
</template>
