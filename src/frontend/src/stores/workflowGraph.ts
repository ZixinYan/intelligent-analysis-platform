...
export function inferSchemaFromDataset(dataset: DatasetDTO, nodeId: string): SchemaInferResultDTO | null {
  ...
  if (columns.length > 0) {
    const fields: FieldSchemaDTO[] = columns.map((col: Record<string, unknown> | { name?: string }, i: number) => {
      const record = col as Record<string, unknown>
      const name = String(record.field ?? record.name ?? Object.values(record)[0] ?? `col${i}`)
...
export const useWorkflowGraphStore = defineStore('workflowGraph', () => {
  ...
  function setViewport(nextViewport: WorkflowViewport, workflowId?: string) {
    graph.value = { ...graph.value, viewport: { ...nextViewport } }
    persistViewport(graph.value.viewport, workflowId)
  }
  ...
  function createNode(meta: NodeMetaDTO, position: XYPosition): WorkflowNode {
    const rawType = getRawNodeType({ type: meta.nodeType, nodeType: meta.nodeType, title: meta.displayName, meta, config: {}, status: 'idle' })
    const businessType = getBusinessNodeType({ type: rawType, nodeType: rawType, title: meta.displayName, meta, config: {}, status: 'idle' })
  ...
  function appendEdge(edge: WorkflowEdge) {
    const exists = graph.value.edges.some(item =>
      ...
      && item.sourceHandle === edge.sourceHandle && item.targetHandle === edge.targetHandle)
    if (exists) return
    graph.value = {
      ...graph.value,
      edges: [...graph.value.edges.map(item => item.selected ? { ...item, selected: false } : item), edge],
    }
  }
  ...
  function calculateInsertedNodePosition(params: {
    ...
    if (params.mode === 'after-node' && params.sourceNode) {
      ...
      return { x: params.sourceNode.position.x + 320, y: params.sourceNode.position.y + yOffset }
    }
    if (params.mode === 'before-node' && params.targetNode) {
      return { x: params.targetNode.position.x - 320, y: params.targetNode.position.y }
    }
  ...
  function addNode(meta: NodeMetaDTO, position: XYPosition = { x: 120, y: 120 }) {
    return appendNode(createNode(meta, position))
  }

  function updateNode(nodeId: string, updater: (node: WorkflowNode) => WorkflowNode) {
    graph.value = { ...graph.value, nodes: graph.value.nodes.map(node => node.id === nodeId ? updater(node) : node) }
  }

  function updateNodeConfig(nodeId: string, config: Record<string, unknown>, status: AnalysisNodeStatus = 'draft') {
    updateNode(nodeId, node => ({
      ...node,
      data: { ...node.data, config, status, preview: buildNodePreview(node.data.meta, config, getBusinessNodeType(node)) },
    }))
  }

  function updateNodeStatus(nodeId: string, status: AnalysisNodeStatus, preview?: string[]) {
    updateNode(nodeId, node => ({ ...node, data: { ...node.data, status, preview: preview ?? node.data.preview } }))
  }

  function updateNodeSchema(nodeId: string, schema: WorkflowNode['data']['schema']) {
    updateNode(nodeId, node => ({ ...node, data: { ...node.data, schema } }))
  }
  ...
  function onNodesChange(changes: NodeChange[]) {
    ...
    if (!changed) return
    graph.value = { ...graph.value, nodes: nextNodes, edges: nextEdges }
    syncSelectionFromGraph()
  }
  ...
  function onEdgesChange(changes: EdgeChange[]) {
    ...
    if (!changed) return
    graph.value = { ...graph.value, edges: nextEdges }
    syncSelectionFromGraph()
  }
  ...
  function onConnect(connection: Connection) {
    ...
    if (sourceNode && !sourceNode.data.schema && sourceNode.data.debugResult?.result?.dataset) {
      ...
      if (schema) updateNodeSchema(connection.source, schema)
    }
  ...
  function insertNodeAfter(params: { sourceNodeId: string; meta: NodeMetaDTO; sourceHandle?: string }) {
    ...
    const insertedNode = addNode(params.meta, calculateInsertedNodePosition({ mode: 'after-node', sourceNode, sourceHandle: params.sourceHandle }))
    onConnect({ source: sourceNode.id, sourceHandle: params.sourceHandle ?? 'output', target: insertedNode.id, targetHandle: 'input' })
  ...
  function insertNodeBefore(params: { targetNodeId: string; meta: NodeMetaDTO; targetHandle?: string }) {
    ...
    const insertedNode = addNode(params.meta, calculateInsertedNodePosition({ mode: 'before-node', targetNode }))
    onConnect({ source: insertedNode.id, sourceHandle: 'output', target: targetNode.id, targetHandle: params.targetHandle ?? 'input' })
  ...
  function insertNodeOnEdge(params: { edgeId: string; meta: NodeMetaDTO }) {
    ...
    const insertedNode = addNode(params.meta, calculateInsertedNodePosition({ mode: 'on-edge', sourceNode, targetNode }))
    graph.value = { ...graph.value, edges: graph.value.edges.filter(item => item.id !== edge.id) }
    appendEdge(createEdge({ source: sourceNode.id, sourceHandle: edge.sourceHandle ?? 'output', target: insertedNode.id, targetHandle: 'input' }))
    appendEdge(createEdge({ source: insertedNode.id, sourceHandle: 'output', target: targetNode.id, targetHandle: edge.targetHandle ?? 'input' }))
    if (edge.condition) {
  ...
  function buildUpstreamInputs(nodeId: string): Record<string, unknown> {
    ...
    for (const edge of graph.value.edges) {
      ...
      if (result) collected[edge.source] = result
    }
  ...
  function serialize(workflowName: string) {
    return graphToSaveRequest(graph.value, workflowName)
  }

  function hydrateGraph(definition: WorkflowDefinitionDTO) {
